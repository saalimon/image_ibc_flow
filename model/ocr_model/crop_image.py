...
test_images_dir = "/app/input/images"
...
def cropping():
    ...
    # Check if the output crops directory exists
    if not os.path.exists(output_crops_dir):
        print(f"Output crops directory '{output_crops_dir}' does not exist.")
        return
    # Check if the output crops directory is empty
    ...
    for image_file in tqdm(image_files, desc="Processing images", unit="image"):
        ...
        # Process the results
        for result_idx, (box, cls) in enumerate(
            zip(results[0].boxes.xyxy, results[0].boxes.cls)
        ):
    ...
    return f"Cropping completed. {len(results)} images processed."
